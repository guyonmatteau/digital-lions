...
async def add_community(community: Community, db: Session = Depends(get_db)):
    if db.query(schemas.Community).filter(schemas.Community.name == community.name).first():
        return JSONResponse(
...
async def update_community(community_id: int, community: Community, db: Session = Depends(get_db)):
    db_community = db.query(schemas.Community).filter(schemas.Community.id == community_id).first()
